...
import skt1.dtos.NotDto;

public class NotDao extends SqlMapConfig {
	
	private String nameSpace="skt1.notice.";
	...
	//글목록의 페이지 개수 구하기
			...
			public List<NotDto> getAllListPage(String pnum,String id,String title,String content){
			...
			public List<NotDto> notAllListPage(String pnum,String id,String title,String content){
				List<NotDto> list=new ArrayList<>();
				Map<String, String> map=new HashMap<>();
				map.put("pnum", pnum);
				map.put("id", id);
				...
				try {
					SqlSessionFactory sqlSessionFactory=getSqlSessionFactory();
					sqlSession=sqlSessionFactory.openSession(true);//autocommit->true
					list=sqlSession.selectList(nameSpace+"noticelist");//list[hkdto,hkdto...]
				} catch (Exception e) {
					e.printStackTrace();
				}finally {
					sqlSession.close();
		...
		public boolean insertBoard(NotDto dto) {
			int count=0;
			SqlSession sqlSession=null;
			try {
		...
		//글상세보기(AnsDto반환)
		public NotDto getBoard(int seq){
		...
		public boolean updateBoard(NotDto dto) {
			int count=0;
			SqlSession sqlSession=null;
			try {
				sqlSession=getSqlSessionFactory().openSession(true);
		...
		public boolean replyBoard(NotDto dto) {
			int count=0;
			SqlSession sqlSession=null;
			try {                                         //transaction처리
		...
		public NotDto getContent(int seq) {
			NotDto dto=new NotDto();
			SqlSession sqlSession=null;
			try {
				sqlSession=getSqlSessionFactory().openSession(true);
